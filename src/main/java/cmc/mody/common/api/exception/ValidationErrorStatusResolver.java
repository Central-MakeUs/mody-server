...
final class ValidationErrorStatusResolver {
    ...
    private static final Map<String, ErrorStatus> STATUS_BY_MESSAGE = Map.ofEntries(
        ...
        Map.entry("체중 기록일은 필수입니다.", ErrorStatus.MEMBER_WEIGHT_INVALID),
        Map.entry("체중 기록일은 오늘 또는 과거 날짜여야 합니다.", ErrorStatus.MEMBER_WEIGHT_INVALID),
        Map.entry("체중은 필수입니다.", ErrorStatus.MEMBER_WEIGHT_INVALID),
        Map.entry("체중은 20kg 이상이어야 합니다.", ErrorStatus.MEMBER_WEIGHT_INVALID),
        Map.entry("체중은 300kg 이하여야 합니다.", ErrorStatus.MEMBER_WEIGHT_INVALID),
        Map.entry("체중은 소수점 둘째 자리까지 입력해주세요.", ErrorStatus.MEMBER_WEIGHT_INVALID),
