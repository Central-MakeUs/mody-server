...
1. 앱은 KST 기준 해당 날짜의 최종 누적 걸음 수를 전송한다.
2. 서버는 `(group_challenge_id, member_id, recorded_on)` 기준으로 `step_record`를 생성하거나 갱신한다.
3. 같은 날짜를 재전송하면 해당 날짜 값만 덮어쓰므로 이전 날짜 기록은 유지되고 중복 합산되지 않는다.
4. 저장 후 그룹 챌린지 전체 걸음 수가 목표 이상이면 그룹 챌린지를 `COMPLETED`로 전환한다.
