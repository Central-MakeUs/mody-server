...
6. 과거 각 날짜에 참여 중이었던 모든 그룹원이 같은 날짜에 1개 이상 기록한 이력이 한 번이라도 있으면 `hasStartedStreak = true`를 반환한다. 이 값은 이번 달이 아닌 그룹 가입 이력의 최초 가입일부터 조회하며, 이후 새 구성원이 합류해도 기존 시작 이력은 유지된다.
7. 이번 달 운동 기록의 `exerciseDurationMinutes` 합계를 반환한다.
8. 이번 달 `COMPLETED` 상태가 된 그룹 챌린지 개수를 반환한다.
