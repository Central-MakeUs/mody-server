...
알림 목록 응답과 FCM data payload에는 동일한 `link`를 포함한다.
클라이언트는 알림을 탭했을 때 `link`를 앱 내부 경로로 해석해 이동한다.

| 타입 | link | 이동 화면 |
| --- | --- | --- |
| GROUP_MEMBER_JOINED | `/groups/{groupId}/home` | 해당 그룹 홈 |
| EXERCISE_REMINDER | `/records/exercise` | 운동 기록 입력 |
| MEAL_REMINDER | `/records/meal` | 식사 기록 입력 |
| COMMENT_CREATED | `/records/{recordId}` | 댓글이 달린 게시글 상세 |
| GROUP_RECORD_STREAK_RISK | `/groups/{groupId}/home` | 최근 방문한 그룹 홈 |
| BUDDY_NUDGE | `/groups/{groupId}/home` | 콕 찌른 회원이 있는 그룹 |
| STEP_CHALLENGE_COMPLETED | `/groups/{groupId}/challenges/step` | 해당 그룹 걸음 수 챌린지 |
| WEEKLY_CHALLENGE_COMPLETED | `/groups/{groupId}/challenges?tab=weekly` | 해당 그룹 챌린지 탭 |

`link`는 알림 생성 시점에 계산해 저장한다.
과거 데이터처럼 `link`가 비어 있으면 `notification_type`, `reference_type`, `reference_id` 기준으로 fallback 계산한다.
...
- 운동 일정은 최소 개수 제한 없이 회원이 등록한 목록을 기준으로 한다.
...
- 재시도 가능한 일시 실패는 Slack으로 전송하지 않는다.
- 재시도 한도를 초과해 `FAILED`로 확정된 알림만 Slack 운영 알림 채널로 전송한다.
...
9. `FAILED`로 확정되면 알림 id, 수신자 id, 알림 타입, 참조 정보, retry count, 예외 정보를 Slack으로 전송한다.
...
- FCM data payload에는 `notificationId`, `type`, `referenceType`, `referenceId`, `link`, `imageKey`를 포함한다.
...
발송 최종 실패 알림은 기존 500 에러 알림 채널을 재사용하므로 아래 설정도 함께 사용한다.

```yaml
error-alert:
  slack:
    enabled: ${SLACK_ERROR_ALERT_ENABLED:false}
    webhook-url: ${SLACK_ERROR_ALERT_WEBHOOK_URL:}
    max-message-length: ${SLACK_ERROR_ALERT_MAX_MESSAGE_LENGTH:3500}
```

별도 notification 전용 webhook은 두지 않는다.
...
- FCM 발송 최종 실패 Slack 전송 실패: warn 로그만 남기고 sender 트랜잭션에는 영향을 주지 않는다.
...
- retry 가능한 실패에는 Slack 알림을 요청하지 않는다.
- retry count가 한도를 넘으면 Slack 실패 알림을 요청한다.
- Slack 알림 설정이 꺼져 있으면 실패 알림은 no-op으로 처리한다.
