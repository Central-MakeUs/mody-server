...
- `member`: 닉네임, 생년월일, 목표 체중, 프로필 이미지 key, 그룹 온보딩 완료 이력, 건강 앱 연동 상태를 가진다. 현재 체중은 최신 `weight_record`로 계산한다.
...
- `group_member`: 한 회원이 여러 그룹에 속할 수 있도록 회원-그룹 관계와 가입/탈퇴 시각을 저장한다. 회원별 활성 그룹 수는 4개, 그룹별 활성 멤버 수는 12명으로 제한하며, 그룹별 표시 닉네임/프로필 이미지 key를 가진다.
...
- `notification_setting`, `exercise_schedule`: 식사/운동 알림, 코멘트 알림, 챌린지 알림과 요일별 운동 일정을 저장한다. 운동 일정은 `day_of_week`와 `scheduled_time`으로 표현한다.
